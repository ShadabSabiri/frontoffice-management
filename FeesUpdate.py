...
def UpdateFees():
 ...
 z.place(x=1,y=1)
 z.create_text(700,100,text='   FEES UPDATE  ',font="Times 40 bold")
 def cleardata():
     t.destroy()
 ...
 def data_update():
     ...
     db=pymysql.connect(host='localhost',user='root',password='root', database="frontOM")
     cur=db.cursor()
     s="update fees set Amount=%d,INsOne=%d,InsTwo=%d where Feeid='%s' "%(y,pp,rr,x)
     cur.execute(s)
     db.commit()
 ...
 h=Entry(t,width=30)
 h.place(x=700,y=440)
 bt=Button(t,text=' Update',command=data_update,font="Times 10 bold",bg="green" ,fg="white")
