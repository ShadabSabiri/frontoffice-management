...
def UpdateRegistration():
  ...
  z.place(x=1,y=1)
  z.create_text(800,70,text='   REGISTRATION  UPDATE   ',font="Times 40 bold")
  def cleardata():
      t.destroy()
  def updatedata():
    ...
    st="update Registration set Sname='%s' ,Address='%s' , City ='%s' ,Phone='%s' ,Email='%s' ,Courseid='%s' , Feeid='%s' where Resid='%s' " %(c,e,g,i,k,m,o,a)
    cur.execute(st)
  ...
  def finddata():
            ...
            if len(data)==0:
                ...
                s="select * from Registration where Resid='%s'"%(x)
                cur.execute(s)
                data=cur.fetchone()
                d.insert(0,data[1])
                f.insert(0,data[2])
